...
app.use("/login",(req,res) => {
  request('https://cas.its.msstate.edu/cas/serviceValidate?service=http://timeclock.library.msstate.edu/login&ticket='+req.param('ticket'), function (error, response, body) {
    ...
    var token = jwt.sign({ user: currentUser }, 'Justin-Is-The-Food-Devil'); // Token expires in 1m
    res.redirect('/?'+token);
  }) 
});
...
app.use(bodyParser.urlencoded({ extended: true }));
 ...
 app.use('/db', textParser, (req,res) =>{		
  var connection = mysql.createConnection({host:'130.18.123.31', user: 'web_dev', password: 'Let$TryTh!$again2017', database: 'timeclock'});		 		

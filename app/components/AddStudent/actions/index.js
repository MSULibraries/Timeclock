...
if (action == 'CHECK-STUDENT-STATUS'){
  ...
  if(state.status === "true"){
    return{
		type: "SUBMIT-STUDENT",
    NetID: state.NetID,
    query: "INSERT INTO student_hours (NetID, 9Digit, FirstName, LastName, EmployeeType, Sex, Race, Department1, Department2, Department3, Department4 ) VALUES('"+state.NetID+"','"+state.NINEdigit+"','"+state.firstName+"','"+state.lastName+"','"+state.employeeType+"','"+state.sex+"','"+state.race+"','"+state.dept1+"','"+state.dept2+"','"+state.dept3+"','"+state.dept4+"') "
	}
 }
 else{
    return{
